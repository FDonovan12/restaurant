...
public class AdminMenuController {
    ...
    @GetMapping(path = UrlRoute.URL_ADMIN_MENU)
    public ModelAndView index(ModelAndView mav) {
        mav.setViewName("admin/menu/index");
        mav.addObject("menus", menuService.findAll());
        return mav;
    }

    @GetMapping(value = UrlRoute.URL_ADMIN_MENU + "/{field}")
    public ModelAndView show(ModelAndView mav, @PathVariable String field) {
        Menu menu = menuService.getByField(field);

        mav.setViewName("admin/menu/show");
        mav.addObject("menu", menu);
        return mav;
    }

    @GetMapping(path = UrlRoute.URL_ADMIN_MENU_NEW)
    ...
    @GetMapping(path = UrlRoute.URL_ADMIN_MENU_EDIT + "/{id}")
    ...
    @PostMapping(path = UrlRoute.URL_ADMIN_MENU_NEW)
    ...
    @PutMapping(path = UrlRoute.URL_ADMIN_MENU_EDIT + "/{id}")
    public ModelAndView formHandler(
            @Valid @ModelAttribute("menu") MenuDTO menuDTO,
            BindingResult result,
            ModelAndView mav,
    ...
    private ModelAndView getFormByDTO(ModelAndView mav, MenuDTO dto, String uri, boolean isEdit) {
        mav.setViewName("admin/menu/form");
    ...
    private ModelAndView formHandle(BindingResult result, ModelAndView mav, MenuDTO dto, Long id) {
        ...
        mav.setViewName("redirect:" + UrlRoute.URL_ADMIN_MENU);
        return mav;
    }
