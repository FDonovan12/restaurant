...
public class AdminPlatController {
    ...
    @GetMapping(path = UrlRoute.URL_ADMIN_PLAT)
    public ModelAndView index(ModelAndView mav) {
        mav.setViewName("admin/plat/index");
        mav.addObject("plats", platService.findAll());
        return mav;
    }

    @GetMapping(value = UrlRoute.URL_ADMIN_PLAT + "/{field}")
    public ModelAndView show(ModelAndView mav, @PathVariable String field) {
        Plat plat = platService.getByField(field);

        mav.setViewName("admin/plat/show");
        mav.addObject("plat", plat);
        return mav;
    }

    @GetMapping(path = UrlRoute.URL_ADMIN_PLAT_NEW)
    ...
    @GetMapping(path = UrlRoute.URL_ADMIN_PLAT_EDIT + "/{id}")
    ...
    @PostMapping(path = UrlRoute.URL_ADMIN_PLAT_NEW)
    ...
    @PutMapping(path = UrlRoute.URL_ADMIN_PLAT_EDIT + "/{id}")
    public ModelAndView formHandler(
            @Valid @ModelAttribute("plat") PlatDTO platDTO,
            BindingResult result,
            ModelAndView mav,
    ...
    private ModelAndView getFormByDTO(ModelAndView mav, PlatDTO dto, String uri, boolean isEdit) {
        mav.setViewName("admin/plat/form");
    ...
    private ModelAndView formHandle(BindingResult result, ModelAndView mav, PlatDTO dto, Long id) {
        ...
        mav.setViewName("redirect:" + UrlRoute.URL_ADMIN_PLAT);
        return mav;
    }
